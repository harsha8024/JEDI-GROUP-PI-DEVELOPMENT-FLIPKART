...
public class FlipfitApplication {
	 ...
	 * Handle user login
	 */
	private static void handleLogin(Scanner scanner, GymUserInterface userService, GymCustomerInterface customerService,
			GymOwnerInterface ownerService, GymAdminInterface adminService) {
		System.out.println("\n--- Login ---");
	...
	private static void handleGuestMenu(Scanner scanner, GymCustomerInterface customerService) {
		...
		while (!back) {
			...
			case 2:
				...
				currentGyms.stream().filter(g -> g.getGymId().equalsIgnoreCase(inputId)).findFirst()
						.ifPresentOrElse(g -> {
							System.out.println("----------------------------");
	...
	private static void handleCustomerMenu(Scanner scanner, GymCustomerInterface customerService) {
		...
		while (!back) {
			...
			case 7:
				...
				// The missing line that actually calls the service!
